...
def app(CDD_TOKEN='None'):
    ...
    # Allow user to enter hTIAC_g
    with st.form('hTIAC_g'):
        st.write('Enter human residence time per gram tissue [MBq*h/MBq]')
        data_input = {}
        for ie,tissue_ie in enumerate(tissues):
            data_input[tissue_ie] = list()
        for j, (k, v) in enumerate(data_input.items()):
            v.append((st.text_input(f'{k} {htiac_key}',key=str(j)+str(k))))
    ...
    if st.session_state.tiac_calculated: 
        ...
        for ik, tissue in enumerate(tissues):
            try:
                htiac = float(data_input[tissue][0])
            except:
                st.error(f'Please enter hTIAC for {tissue} (hTIAC per gram of organ)')
                htiac = 0.
            ...
            htiac_org = float(htiac) * org_weight_human
        ...
        if st.session_state.calc_scaling:
            ...
            st.write(f'Dosimetry was calculated from hTIAC_g')
